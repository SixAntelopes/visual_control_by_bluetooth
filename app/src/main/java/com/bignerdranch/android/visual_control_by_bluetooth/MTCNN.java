...
public class MTCNN {
    ...
    public Bitmap processImage(Bitmap bp, SimpleBluetooth mSimpleBluetooth, TextView mTextView, Switch mSwitch){
        try {
            Vector<Box> boxes=this.detectFaces(bp,350);
            //  找最大框(之后用人脸判断代替)
            int controler = 0;
            int con_id = 0;
            for(int i = 0;i<boxes.size();i++)
            {
                Rect r = boxes.get(i).transform2Rect();
                int heigh = r.top - r.bottom;
                int width = r.left - r.right;
                if(width * heigh > controler)
                {
                    controler = width * heigh;
                    con_id = i;
                }
            }
            //画矩形和关键点
            Utils.drawRect(bp,boxes.get(con_id).transform2Rect());
            Utils.drawPoints(bp,boxes.get(con_id).landmark);
            Utils.drawJudge(bp,boxes.get(con_id).landmark);
            //  最大框找到后，定义控制方式

            mSwitch.setNowResult(Utils.newJudge(boxes.get(con_id).transform2Rect(), boxes.get(con_id).landmark));
            Log.d(TAG,"result==="+mSwitch.getNowResult());

//            Rect rect = boxes.get(con_id).transform2Rect();
//            Point[] landmark = boxes.get(con_id).landmark;
...
//
//            double eye = (eyeLeftY+eyeRightY)/2.0;
//            double mouth = (mouthLeftY+mouthRightY)/2.0;
//            double eye2mouth = mouth - eye;
//            Log.d(TAG, "eye2mouth           " + eye2mouth);
//            Log.d(TAG, "(mouth - noseY)           " + (mouth - noseY));
//            Log.d(TAG, "(noseY - eye)           " + (noseY - eye));
//            Log.d(TAG, "(eyeRightX-eyeLeftX)/(noseX-eyeLeftX)           " + (((double)eyeRightX-eyeLeftX)/(noseX-eyeLeftX)));
//            Log.d(TAG, "landmark[1].x           " + boxes.get(con_id).landmark[1].x);
//            Log.d(TAG, "landmark[1].y           " + boxes.get(con_id).landmark[1].y);

//             按下开关了
            if (mSwitch.getNowResult() == 4 && mSwitch.getLastResult() == 4) {
                mSwitch.setmSwitch(!mSwitch.ismSwitch());
            }
            //开关是关的
            if (!mSwitch.ismSwitch()) {
                mTextView.setText("停止");
                mSimpleBluetooth.getConnectedThread().write("I");
                return bp;
            }
            //接收了相同的指令
            if (mSwitch.getNowResult() == mSwitch.getLastResult()) {
                return bp;
            }

            mSwitch.setLastResult(mSwitch.getNowResult());
            mSimpleBluetooth.getConnectedThread().write("I");

            String ans;
            /** 使用 SendOrder() 来传输数据 **/
            switch (mSwitch.getNowResult())
            {
                case 0:
                    ans = "前进";
